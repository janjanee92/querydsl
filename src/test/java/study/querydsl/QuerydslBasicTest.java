...
public class QuerydslBasicTest {
    ...
    /**
     *  회원과 팀을 조인하면서, 팀 이름이 teamA인 팀만 조인, 회원은 모두 조
...
//                .on(team.name.eq("teamA"))
    ...
    private List<Member> searchMember2(String usernameCond, Integer ageCond) {
        return queryFactory
                .selectFrom(member)
//                .where(usernameEq(usernameCond), ageEq(ageCond))    //  where에 null이 들어오면 무시됨.
                .where(allEq(usernameCond, ageCond))    //  위와 동일 코드 이렇게 한번에 연결한 메소드도 생성가능!
                .fetch();
    }
